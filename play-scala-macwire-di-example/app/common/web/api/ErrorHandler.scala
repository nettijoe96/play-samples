...
class ErrorHandler (
  ...
  private val logger = LoggerFactory.getLogger(classOf[ErrorHandler])
  ...
  override def onServerError(request: RequestHeader, exception: Throwable) = {
    logger.error(s"Uncaught: $exception", exception)
    jsError(errorInternalServer(exception.getMessage)(request2Messages(request)), request)
  }

  private def (error: ApiError, request: RequestHeader): Future[Result] = Future.successful {
    error.saveLog(ApiRequestHeaderImpl(request)).toResult(request)
  }
